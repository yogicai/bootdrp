...
public class CollectionUtils {
    ...
    public static <T, U> List<U> convertList(Collection<T> from, BiFunction<T, Integer, U> func) {
        if (CollUtil.isEmpty(from)) {
            return new ArrayList<>();
        }
        return IntStream.range(0, from.size())
                .mapToObj(i -> func.apply(new ArrayList<>(from).get(i), i)).filter(Objects::nonNull).collect(Collectors.toList());
    }
