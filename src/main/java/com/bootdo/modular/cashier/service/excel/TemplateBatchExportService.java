package com.bootdo.modular.cashier.service.excel;
 ...
 *
 * @author caiyz
 * @since 2026-08-11 15:41
 */
public class TemplateBatchExportService extends ExcelExportService implements IWriter<Workbook> {
    ...
    public void init(Workbook workbook, ExportParams entity, Class<?> pojoClass) {
        Assert.isInstanceOf(XSSFWorkbook.class, workbook, "ExcelBatchExportServer only support XSSFWorkbook");

        List<ExcelExportEntity> excelParams = createExcelExportEntityList(entity, pojoClass);
        init(workbook, entity, excelParams);
    }
    ...
    protected void insertDataToSheet(Workbook workbook, ExportParams entity, List<ExcelExportEntity> entityList, Collection<?> dataSet, Sheet sheet) {
        try {
            ...
            rowHeight = entity.getHeight() != 0 ? entity.getHeight() : getRowHeight(excelParams);
            setCurrentIndex(1);
        } catch (Exception e) {
