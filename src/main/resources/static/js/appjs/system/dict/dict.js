	...
	$exampleTable.bootstrapTable(
			{
				...
				columns : [
					...
					{
						title: '状态', field: 'delFlag', formatter: (value, index) => {
							if (value === 0) {
					...
					},
					{
						title: '操作', field: 'id', align: 'center', formatter: (value, row, index) => {
							...
							let a = `<a class="btn btn-warning btn-sm ${s_add_h}" href="#" title="增加下級" onclick="remove('${row.id}')"><i class="fa fa-remove"></i></a> `;
							let d = `<a class="btn btn-success btn-sm ${s_remove_h}" href="#" title="增加" onclick="addD('${row.type}', '${row.description}')"><i class="fa fa-plus"></i></a> `;
							return e + a + d;
						}
					}
				]
