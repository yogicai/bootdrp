...
$(function() {
    ...
    dataForm  = $('#data_form');
    mask = $('#mask');
    utils.loadCategory(["CUSTOMER_DATA","ACCOUNT_DATA","USER_DATA"], ["consumerId","settleAccountTotal","billerId"], [{width:"200px"},{width:"185px"},{width:"200px"},]);
});
...
function collectValueTotal() {
    ...
    var expenseFeeTotal=$("#expenseFeeTotal").val() || 0;
...
function save(add) {
    ...
    if (order.consumerId == "" || order.billerId == "" || order.billDate == "" || order.settleAccountTotal == "" || amountOrder.totalObj.totalQtyTotal <= 0) {
        layer.msg((order.consumerId == "" ? "[客户信息]" : "") + (order.billerId == "" ? "[销售人员信息]" : "") + (order.billDate == "" ? "[单据日期]" : "") + (order.settleAccountTotal == "" ? "[结算帐户]" : "")  + (amountOrder.totalObj.totalQtyTotal <= 0 ? "[合计数量]" : "") + "不能为空！");
        return;
    }
...
function insertHead(data) {
    if (!data) return;
    $("#consumerId").val(data.no).trigger("chosen:updated");
}
