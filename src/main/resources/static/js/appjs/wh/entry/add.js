...
function add() {
    ...
    $(ids).each(function (index, id){
        ...
        selectData.push({ "entryName": row.name,"entryId": row.no,"entryUnit": row.unit,"totalQty": 1,"entryPrice": row.costPrice,"stockNo": row.stockNo,"entryAmount":row.costPrice,"totalAmount":row.costPrice })
    });
...
function getTreeData() {
    $.ajax({
        type: "GET", data: {"type": "PRODUCT"}, url: "/data/category/tree", success: function (tree) {
            loadTree(tree);
        }
    });
}
